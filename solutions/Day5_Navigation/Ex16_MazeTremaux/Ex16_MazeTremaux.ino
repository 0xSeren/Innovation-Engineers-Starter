/*
 *  EXERCISE 16 -- Maze Solving: Tremaux's Algorithm  (SOLUTION)
 *  Full standalone reference: ~/projects/code/MazeSolverTremaux/
 */
#include <Arduino.h>
#include <Wire.h>
...
void loop(){
  ...
  if(n.marks[came]<2) n.marks[came]++;

  int chosen;                                         // SOLUTION to TODO
  if (isNew) {
  ...
  if(n.marks[chosen]<2) n.marks[chosen]++;
  executeRel((chosen-curDir+4)%4);
  gX=nx*CELL_CM; gY=ny*CELL_CM;
}
